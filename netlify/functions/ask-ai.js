exports.handler = async (event) => {
  ...
  if (!apiKey) {
    return {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        error: "Assistant IA non configuré (GEMINI_API_KEY manquante)"
      })
    };
  }
...
Matière : ${subjectName || "non précisée"}.
...
Reste dans le cadre de cette leçon autant que possible.
Si la question sort du sujet de la leçon, réponds quand même brièvement mais rappelle gentiment le lien avec la leçon en cours.
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      return {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          error: "Échec de la requête à l'assistant IA",
          details
        })
      };
    }
