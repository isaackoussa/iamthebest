...
(function(){
  ...
  let authDemoCode = null; // repli si les fonctions Netlify ne sont pas disponibles (aperçu hors ligne)

  function updateTopbarUser(){
    const el = document.getElementById("topbar-user");
    if (!el) return;
    if (currentEmail){
      el.innerHTML = `<span class="user-email">${esc(currentEmail)}</span> <button id="logout-btn" class="btn-link">Se déconnecter</button>`;
  ...
  function loadRemoteThenRender(){
    ...
    fetch(`/api/progress?email=${encodeURIComponent(currentEmail)}`).then(r => r.ok ? r.json() : null).then(remote => {
      if (remote && typeof remote === "object" && Object.keys(remote).length){
        progress = Object.assign({}, progress, remote);
        saveLocal();
      }
    ...
    if (authStep === "email"){
      root.innerHTML = `
        <div class="auth-card">
          ...
          <p class="auth-sub">Entre ton adresse email pour créer ton compte ou retrouver ta progression. Un code de vérification te sera envoyé.</p>
          ...
          <button class="btn btn-primary auth-btn" id="auth-send-btn" ${authLoading ? "disabled" : ""}>${authLoading ? "Envoi en cours..." : "Recevoir mon code"}</button>
        </div>
      `;
      const input = document.getElementById("auth-email");
      input.focus();
      input.addEventListener("keydown", e => { if (e.key === "Enter") sendCode(); });
      document.getElementById("auth-send-btn").addEventListener("click", sendCode);
    } else {
      root.innerHTML = `
        <div class="auth-card">
          <div class="auth-badge">✉️</div>
          <h1 class="auth-title">Vérifie ton email</h1>
          <p class="auth-sub">On a envoyé un code à <strong>${esc(authPendingEmail)}</strong>. Entre-le ci-dessous.</p>
          ${authDemoCode ? `<p class="auth-demo">Mode aperçu hors ligne — pas d'email réel envoyé. Ton code : <strong>${authDemoCode}</strong></p>` : ""}
          <input type="text" id="auth-code" class="auth-input auth-code-input" placeholder="123456" maxlength="6" inputmode="numeric">
          ${authError ? `<p class="auth-error">${esc(authError)}</p>` : ""}
          <button class="btn btn-primary auth-btn" id="auth-verify-btn" ${authLoading ? "disabled" : ""}>${authLoading ? "Vérification..." : "Valider le code"}</button>
          <button class="btn-link auth-back" id="auth-back-btn">← Changer d'email</button>
        </div>
      `;
      ...
      document.getElementById("auth-back-btn").addEventListener("click", () => {
        authStep = "email"; authError = ""; authDemoCode = null; render();
      });
    }
  }
  ...
  function sendCode(){
    ...
    authPendingEmail = email; authError = ""; authLoading = true; authDemoCode = null; render();
    ...
    fetch("/api/send-code", {
      ...
      authLoading = false;
      if (ok){ authStep = "code"; authError = ""; }
      else { authError = (data && data.error) || "Échec de l'envoi du code."; }
      render();
    }).catch(() => {
      // Pas de backend disponible (ex: aperçu hors ligne) : mode démo local
      authDemoCode = Math.floor(100000 + Math.random()*900000).toString();
      window.__imthebestDemoCode = authDemoCode;
      authLoading = false; authStep = "code"; authError = "";
      render();
    });
  }
  ...
  function verifyCode(){
    ...
    if (!code){ authError = "Entre le code reçu."; render(); return; }
    authLoading = true; authError = ""; render();

    if (authDemoCode){
      // mode démo hors ligne : validation locale uniquement
      authLoading = false;
      if (code === window.__imthebestDemoCode){
        loginSuccess(authPendingEmail);
      } else {
        authError = "Code incorrect."; render();
      }
      return;
    }
    ...
    fetch("/api/verify-code", {
      ...
      authLoading = false;
      if (ok){ loginSuccess(authPendingEmail); }
      else { authError = (data && data.error) || "Code incorrect."; render(); }
    }).catch(() => {
      authLoading = false; authError = "Connexion impossible. Réessaie."; render();
    });
  }
  ...
  function loginSuccess(email){
    ...
    authStep = "email"; authPendingEmail = ""; authError = ""; authDemoCode = null;
  ...
  function renderSubject(key){
    ...
    root.innerHTML = `
      ...
      ${SITUATIONS[key] ? `<a class="btn btn-ghost situations-link" style="--subj-color:${subj.color}" href="#/situations/${key}">📋 Situations d'évaluation</a>` : ""}
